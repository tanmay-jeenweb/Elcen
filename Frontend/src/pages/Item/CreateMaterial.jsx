...
export default function CreateMaterial() {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        // Fetch dropdowns
        const [groupRes, unitRes] = await Promise.all([
          getMaterialGroups(),
          getUnits(),
        ]);
        ...
        if (isEditMode) {
          ...
          if (mat) {
            const isStandardType = MATERIAL_TYPES.includes(mat.material_type);
            setForm({
              ...
              materialType: mat.material_type ? (isStandardType ? mat.material_type : "Other") : "",
            ...
            if (mat.material_type && !isStandardType) {
              setCustomMaterialType(mat.material_type);
            }
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.materialType === "Other" && !customMaterialType.trim()) {
      toast.error("Custom Material Type value is required.");
      return;
    }

    setSaving(true);
    try {
      const selectedType = form.materialType === "Other" ? customMaterialType.trim() : form.materialType;
      const payload = {
  ...
  const inputCls =
    "w-full border border-slate-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#043464] focus:border-[#043464] transition-colors text-slate-800 bg-white";
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              {/* Specify Material Type (Conditional) */}
              {form.materialType === "Other" && (
                <div>
                  <label className={labelCls}>
                    Specify Material Type <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="text"
