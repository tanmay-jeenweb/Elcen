...
import { createProcess, getProcesses, updateProcess, deleteProcess } from "../../../api/processMasterApi";
...
export default function ProcessMaster() {
  ...
  const loadProcesses = async () => {
    ...
    try {
      ...
      toast.error("Unable to load processes. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddProcess = async (event) => {
    event.preventDefault();
    if (!newName.trim()) {
      toast.error("Enter a valid process name.");
      return;
    }

    setSaving(true);
    try {
      await createProcess({ processName: newName.trim(), logs: "" });
      ...
      toast.error(serverMessage || "Unable to add process. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateProcess = async (id) => {
    ...
    setSaving(true);
    try {
      await updateProcess(id, { processName: editingName.trim() });
      ...
      toast.error(err?.response?.data?.message || "Unable to update process.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteProcess = async (id) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || "Unable to delete process.");
    } finally {
      setSaving(false);
    }
  };
    ...
    const cols = [
      { key: 'id', label: 'ID', minWidth: '60px' },
      {
        ...
        render: (row) => editingId === row.id ? (
          ...
          <span className="font-semibold text-blue-900">{row.process_name}</span>
        )
      }
    ];

    const canUpdate = hasPermission("process_master", "update");
    const canDelete = hasPermission("process_master", "delete");
        ...
        render: (row) => (
          <div className="flex items-center justify-start gap-2">
            {editingId === row.id ? (
              <>
                <button
                  onClick={() => handleUpdateProcess(row.id)}
                  disabled={saving}
              ...
              <>
                {canUpdate && (
                  <button
                    onClick={() => handleStartEdit(row.id, row.process_name)}
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 font-sans text-slate-900">
      ...
      <main className="flex-1 flex flex-col w-full max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <DataTable
          tableId="process_master"
          title="Process Master"
          data={processes}
          columns={columns}
          loading={loading}
          searchPlaceholder="Search processes..."
          actionButton={hasPermission("process_master", "write") ? (
            <button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl animate-in zoom-in duration-200">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold text-slate-900">Add Process</h2>
                  <p className="text-sm text-slate-500">Create a new process master entry.</p>
                </div>
                <button
                  type="button"
              ...
              <form className="space-y-4" onSubmit={handleAddProcess}>
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-700">Process Name</label>
                  <input
                    type="text"
                    value={newName}
