...
import { getAllCustomers, deleteCustomer, toggleCustomerActive } from "../../../api/customerApi";
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-[#043464] to-[#065a9e] text-white">
                    <div>
                        <h2 className="text-lg font-bold">{customer.customer_name}</h2>
                        <p className="text-blue-200 text-sm">{customer.customer_code}</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                            customer.customer_status === 'Approved'
                                ? 'bg-emerald-100 text-emerald-700'
                                : 'bg-amber-100 text-amber-700'
                        }`}>
                            {customer.customer_status || 'Not Approved'}
                        </span>
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                            customer.active !== false && customer.active !== 0
                                ? 'bg-emerald-100 text-emerald-700'
                                : 'bg-rose-100 text-rose-700'
                        }`}>
                            {customer.active !== false && customer.active !== 0 ? 'Active' : 'Inactive'}
                        </span>
                ...
                <div className="overflow-y-auto p-6">
                    ...
                    <Section title="1. General Information">
                        ...
                        <Field label="Contact Phone"  value={customer.contact_phone} />
                        ...
                        <Field label="Status"         value={customer.customer_status} />
                        <Field label="Time Zone"      value={customer.time_zone} />
                    </Section>

                    {/* 2. Bank Details */}
                    <Section title="2. Bank Details">
                        <Field label="Bank Name"           value={customer.bank_name} />
                        <Field label="Account Number"      value={customer.bank_account_number} />
                        <Field label="IFSC Code"           value={customer.bank_ifsc} />
                        <Field label="Cheque Printing Name" value={customer.cheque_printing_name} />
                        <Field label="PAN Number"          value={customer.pan_no} />
                    </Section>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-4">
                            <div>
                                ...
                                <p className="text-sm text-slate-800 font-mono font-medium">{customer.gst_no || <span className="text-slate-300 font-sans">—</span>}</p>
                            </div>
                        </div>
                        {documents.length > 0 && (
                            <div className="border border-slate-200 rounded-lg overflow-hidden">
...
export default function CustomerMaster() {
    ...
    const loadCustomers = async () => {
        setLoading(true);
        try {
            const res = await getAllCustomers(showInactive);
    ...
    const handleToggleActive = async (id, currentActive) => {
        ...
        if (!window.confirm(`Are you sure you want to ${newState ? 'activate' : 'deactivate'} this customer?`)) return;
        setSaving(true);
        try {
            await toggleCustomerActive(id, newState);
    ...
    const columns = useMemo(() => {
        const cols = [
            { key: "customer_code",   label: "Code",        minWidth: "100px" },
            { key: "customer_name",   label: "Customer Name", minWidth: "160px",
              render: (row) => <span className="font-semibold text-slate-900">{row.customer_name}</span> },
            ...
            { key: "customer_status", label: "Approval",    minWidth: "120px",
              render: (row) => (
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                    row.customer_status === 'Approved'
                        ? 'bg-emerald-100 text-emerald-700'
                        : 'bg-amber-100 text-amber-700'
                }`}>
                    {row.customer_status || 'Not Approved'}
                </span>
              )
            },
            ...
            render: (row) => {
                ...
                return (
                    <div className="flex items-center gap-1.5">
                        ...
                        {canUpdate && (
                            <button
                                onClick={() => navigate(`/admin/customers/edit/${row.id}`)}
                                className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#bcccdc] bg-[#f0f4f8] text-[#043464] hover:bg-[#e6ebf0]"
                                title="Edit"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="h-4 w-4">
    ...
    return (
        <div className="flex-1 flex flex-col bg-slate-50 font-sans text-slate-900">
                    ...
                    actionButton={
                        canWrite && (
                            <button
                                ...
                                className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#043464] text-white transition-all hover:bg-[#03274b] shadow-sm hover:shadow"
            ...
            {/* View Modal */}
            {viewingCustomer && (
                <CustomerViewModal customer={viewingCustomer} onClose={() => setViewingCustomer(null)} />
            )}
        </div>
    );
}
